ALTER TABLE weather
  ADD COLUMN solar integer CHECK (solar >= 0);

ALTER TABLE synop
  ADD COLUMN solar_10 integer CHECK (solar_10 >= 0),
  ADD COLUMN solar_30 integer CHECK (solar_10 >= 0),
  ADD COLUMN solar_60 integer CHECK (solar_10 >= 0);

-- Same query as before with solar_XX fields added
...
CREATE MATERIALIZED VIEW current_weather AS
  ...
  JOIN (
    SELECT
      ...
      LAST(solar_10 ORDER BY timestamp) AS solar_10,
  ...
  LEFT JOIN (
    SELECT
      synop.source_id,
      round(AVG(solar_10) * 6)::int AS solar_60,
  ...
  LEFT JOIN (
    SELECT
      synop.source_id,
      round(AVG(solar_10) * 3)::int AS solar_30,
